...
def evaluate(model, data_loader, device):
    ...
    confmatCOM = ConfusionMatrix(task="binary", num_classes=2, normalize='true').to(device)
    ...
    for batch, (data, labels) in enumerate(data_loader):
        ...
        prob_class1 = outputs[:, 1]
        final_predictions = torch.where(prob_class1 > 0.5, torch.tensor(1, device=device), torch.tensor(0, device=device))
        
        # 写入 CSV
        with open(config.data + '.csv', 'a', encoding='utf-8') as f:
            line = "\t".join(map(str, final_predictions.cpu().numpy().tolist()))
            f.write(line + "\t")
