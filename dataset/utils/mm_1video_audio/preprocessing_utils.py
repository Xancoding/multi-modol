    ...
    for hospital_name in os.listdir(data_dir):
        ...
        for folder_name in ['cry', 'non-cry']:
            ...
            for patient_name in os.listdir(folder_dir):
                ...
                video_files, audio_files, labels = [], [], []

                tmp_results = defaultdict(lambda: {'audio_files': [], 'video_files': [], 'labels': []})
                for file_name in os.listdir(patient_dir):
                    # if 'zxs_f' not in file_name:
                    #     continue
                    ...
                    video_clips_dir = os.path.join(patient_dir, f'{base_name}_cam1_video_clips')
                    ...
                    if os.path.isdir(file_path):
                        ...
                        elif 'video_clips' in file_name:
                            tmp_results[base_name]['video_files'] = [
                                os.path.join(video_clips_dir, os.path.splitext(fs)[0])
                                for fs in os.listdir(file_path)]
                    else:
                        if ext == '.avi' and not os.path.exists(video_clips_dir) and file_name.endswith('_cam1.avi'):
                            tmp_results[base_name]['video_files'] = split_video(file_path, clip_length=args.clip_length)
                        # if ext == '.mp4' and not os.path.exists(video_clips_dir):
                        #     tmp_results[base_name]['video_files'] = split_video(file_path, clip_length=args.clip_length)
                        ...
                        # elif ext == '.txt':
                        #     tmp_results[base_name.replace('label_', '')]['labels'] = \
                ...
                for val in tmp_results.values():
                    ...
                    sorted_videos = sorted(val['video_files'], key=lambda x: int(os.path.basename(x).split('_')[1]))

                    audio_files += sorted_audios
                    video_files += sorted_videos
                    if folder_name == 'cry':
                        labels += val['labels']
                    else:
                        labels += [0] * len(val['audio_files'])

                assert len(video_files) == len(audio_files), f'{hospital_name}: {patient_name},' \
                                                             f' ensure video and audio are same length'
                assert len(video_files) == len(labels), f'{hospital_name}: {patient_name}, ' \
                                                        f'ensure video and labels are same length'

                all_files += list(zip(audio_files, video_files))
                all_labels += labels
    ...
    for train_val_idx, test_idx in sgkf.split(all_files, all_labels, groups=groups):
        ...
        with open(save_train_path, 'w') as f:
            for sample_idx in range(len(train_idx)):
                f.write(f'{train_files[sample_idx][0]}\t{train_files[sample_idx][1]}\t{train_labels[sample_idx]}\n')
        with open(save_val_path, 'w') as f:
            for sample_idx in range(len(val_idx)):
                f.write(f'{val_files[sample_idx][0]}\t{val_files[sample_idx][1]}\t{val_labels[sample_idx]}\n')
        with open(save_test_path, 'w') as f:
            for sample_idx in range(len(test_idx)):
                f.write(f'{test_files[sample_idx][0]}\t{test_files[sample_idx][1]}\t{test_labels[sample_idx]}\n')
...
def read_data(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            audio_file, video_file, label = line.strip().split('\t')
