...
def split_video(video_path, clip_length=3, resize=None):
    """
    分割视频并可选调整尺寸
    Args:
        video_path: 输入视频路径
        clip_length: 每个片段长度（秒）
        resize: 目标尺寸 (width, height)，如 (480, 300)
    Returns:
        片段路径列表（不带扩展名）
    """
    # Open the video file
    ...
    while start_time <= duration - clip_length:
        ...
        out = cv2.VideoWriter(
            ...
            fourcc, 
            frame_rate, 
            (output_width, output_height)  # 使用调整后的尺寸
        )
        ...
        while current_frame < end_frame:
            ...
            if ret:
                if resize is not None:
                    frame = cv2.resize(frame, (output_width, output_height))  # 调整尺寸
                out.write(frame)
