...
class MultiModalPipeline(BasePipeline):
    ...
    def set_model(self):
        ...
        video_backbone = video_model_dict[self.args.video_model](class_num=2, fps=15, use_fc=True)
        model = get_fusion_model[self.args.fusion_mode](audio_backbone, video_backbone)
        return model
    ...
    def train_one_epoch(self, epoch):
        ...
        for i, data in enumerate(self.dataloader_dict['train']):
            audio_x, video_x, label = data['audio_feature'], data['video_feature'], data['label']
            # forward
            logits = self.model(audio_x, video_x)['logits']
